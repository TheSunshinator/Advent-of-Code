    ...
    return input.asSequence()
        ...
        .mapTo(mutableListOf()) { (identifier, oreRobotCost, clayRobotCost, obsidianRobotOreCost, obsidianRobotClayCost, geodeRobotOreCost, geodeRobotObsidianCost) ->
            Blueprint(
                identifier.toInt(),
                robotCost = buildMap {
                    this[MiningRobot.ORE] = mapOf(Rock.ORE to oreRobotCost.toInt())
                    this[MiningRobot.CLAY] = mapOf(Rock.ORE to clayRobotCost.toInt())
                    this[MiningRobot.OBSIDIAN] = mapOf(
                        Rock.ORE to obsidianRobotOreCost.toInt(),
                        Rock.CLAY to obsidianRobotClayCost.toInt(),
                    )
                    this[MiningRobot.GEODE] = mapOf(
...
private data class State(
    val minutesLeft: Int,
    val robotCount: Map<MiningRobot, Int> = mapOf(MiningRobot.ORE to 1),
    val rockCount: Map<Rock, Int> = emptyMap(),
) {
    val potentialGeodeCount = rockCount.getOrDefault(Rock.GEODE, 0) +
        robotCount.getOrDefault(MiningRobot.GEODE, 0) * minutesLeft +
